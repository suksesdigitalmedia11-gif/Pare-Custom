...
<head>
    ...
    <title>Daftar Produk - Finance - Pare Custom</title>
...
<body class="bg-gray-100">
                ...
                <div class="max-w-7xl mx-auto">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                        ...
                        <div class="flex space-x-3">
                            <div class="relative inline-block text-left" id="exportDropdown">
                                ...
                                <div id="exportMenu" class="hidden absolute right-0 mt-2 w-56 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10 transition-all origin-top-right transform scale-95 opacity-0">
                                    <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                        <a href="{{ route('finance.product.export', array_merge(request()->query(), ['format' => 'xlsx'])) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
                                            <i class="bi bi-file-earmark-spreadsheet text-green-600 mr-3"></i> Export Excel (.xlsx)
                                        </a>
                                        <a href="{{ route('finance.product.export', array_merge(request()->query(), ['format' => 'csv'])) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
                                            <i class="bi bi-filetype-csv text-blue-600 mr-3"></i> Export CSV (.csv)
                                        </a>
                                        <div class="border-t border-gray-100 my-1"></div>
                                        <a href="{{ route('finance.product.export', ['format' => 'xlsx']) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
                            ...
                            <a href="{{ route('finance.product.create') }}" 
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                            <form method="GET" action="{{ route('finance.product.index') }}">
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                    ...
                                    <div class="md:col-span-4 flex gap-2">
                                        ...
                                        <a href="{{ route('finance.product.index') }}" class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition shadow-sm">
                                            Reset
                                        </a>
                                    </div>
                                </div>
                                ...
                                <thead class="bg-gray-50 text-gray-600 font-semibold uppercase text-xs tracking-wider">
                                    <tr>
                                        <th class="px-6 py-4 border-b border-gray-100">
                                            <a href="{{ route('finance.product.index', array_merge(request()->query(), ['sort_by' => 'name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="group flex items-center gap-1 cursor-pointer hover:text-blue-600">
                                                Produk
                                        ...
                                        <tr class="hover:bg-gray-50/80 transition-colors group">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <div class="flex items-center justify-center gap-2">
                                                    <a href="{{ route('finance.product.show', $product) }}" class="p-2 rounded-lg text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="Detail">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                    <a href="{{ route('finance.product.edit', $product) }}" class="p-2 rounded-lg text-gray-500 hover:bg-yellow-50 hover:text-yellow-600 transition-colors" title="Edit">
                                                        <i class="bi bi-pencil"></i>
                                                    </a>
                                                    <button type="button" onclick="confirmDelete('{{ route('finance.product.destroy', $product) }}', '{{ $product->name }}')" 
    ...
    <div id="importModal" class="fixed inset-0 bg-gray-900/50 hidden z-50 flex items-center justify-center transition-opacity opacity-0 pointer-events-none" style="transition: opacity 0.3s ease;">
        <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl transform scale-95 transition-transform duration-300" style="transition: transform 0.3s ease;">
            ...
            <form action="{{ route('finance.product.import') }}" method="POST" enctype="multipart/form-data" id="importForm" class="p-6">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
                        ...
                        <div>
                            ...
                            <a href="{{ route('finance.product.download-template') }}" class="text-xs font-bold text-blue-700 hover:underline">
