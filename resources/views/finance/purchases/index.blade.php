                                    ...
                                    <tr class="border-b hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-3 lg:px-4 py-3">
                                            <div class="flex items-center justify-center gap-1 flex-wrap">
                                                <!-- Detail Button -->
                                                <a href="{{ route('finance.purchases.show', $p) }}"
                                                ...
                                                <!-- Draft: Submit -->
                                                @if($p->status === 'draft')
                                                    <form method="POST" action="{{ route('finance.purchases.submit', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                @if($p->status === 'pending' && in_array(auth()->user()->usertype, ['finance', 'owner']))
                                                    <form method="POST" action="{{ route('finance.purchases.approve', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                @if($p->status === 'printing' && in_array(auth()->user()->usertype, ['finance', 'owner']))
                                                    <form method="POST"
                                                        action="{{ route('finance.purchases.update-status', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                ...
                                                @if(!in_array($p->status, ['selesai', 'cancelled', 'payment', 'proses_jahit', 'printing']))
                                                    <form method="POST" action="{{ route('finance.purchases.cancel', $p) }}"
                                                ...
                                                <!-- Hapus: Finance & Owner -->
                                                @if(in_array(auth()->user()->usertype, ['finance', 'owner']) && !in_array($p->status, ['selesai', 'payment', 'proses_jahit', 'printing']))
                                                    <form method="POST" action="{{ route('finance.purchases.destroy', $p) }}"
                                                ...
                                                <div id="payment-modal-{{ $p->id }}"
                                                    ...
                                                    <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                                                        <h3 class="text-lg font-semibold text-gray-700 mb-4">Proses Pembayaran
                                                            {{ $p->po_number }}</h3>
                                                        <form action="{{ route('finance.purchases.payment', $p->id) }}"
