...
<head>
  ...
  <title>Pergerakan Stok - Admin - Custom Pare</title>
        ...
        <div class="mb-8">
            ...
            <div class="flex border-b border-gray-200 mt-6 overflow-x-auto bg-white rounded-t-xl px-4 pt-2">
                <a href="{{ route('admin.inventory.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('admin.inventory.index') ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-blue-600' }}">Overview</a>
                <a href="{{ route('admin.inventory.stock-ins.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('admin.inventory.stock-ins.*') ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-blue-600' }}">Stok Masuk</a>
                <a href="{{ route('admin.inventory.stock-opnames.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('admin.inventory.stock-opnames.*') ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-blue-600' }}">Stock Opname</a>
                <a href="{{ route('admin.inventory.stock-movements.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('admin.inventory.stock-movements.*') ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-blue-600' }}">Pergerakan Stok</a>
            </div>
        </div>
            ...
            <form method="GET" class="flex flex-col lg:flex-row gap-4">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ $search }}" 
                        class="w-full pl-11 pr-4 py-3 rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm" 
                        placeholder="Cari Produk atau SKU...">
                </div>
                <div class="flex flex-wrap lg:flex-nowrap gap-3">
                    <div class="relative group">
                        <label class="absolute -top-2 left-3 px-1 bg-white text-[10px] font-bold text-blue-600 uppercase tracking-wider">Mulai</label>
                        <input type="date" name="from" value="{{ $from }}" 
                            class="rounded-xl border-gray-200 py-3 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-all">
                    </div>
                    <div class="relative group">
                        <label class="absolute -top-2 left-3 px-1 bg-white text-[10px] font-bold text-blue-600 uppercase tracking-wider">Sampai</label>
                        <input type="date" name="to" value="{{ $to }}" 
                            class="rounded-xl border-gray-200 py-3 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm transition-all">
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 font-bold flex items-center gap-2">
                  ...
                  @forelse($stockMovements as $item)
                      <tr class="hover:bg-blue-50/30 transition-all group cursor-pointer" @click="openModal({{ $item->product_id }})">
                          <td class="px-6 py-4">
                              ...
                              <div class="text-[11px] font-medium text-blue-600 mono mt-1 select-all group-hover:underline">
                                  {{ $item->product->sku ?? '-' }}
                              </div>
                          </td>
                          ...
                          <td class="px-6 py-4 text-right">
                              <button class="w-8 h-8 rounded-full bg-white border border-gray-200 text-gray-400 flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5 mono">
                            SKU: <span class="font-bold text-blue-600" x-text="details.product.sku"></span> | 
                                            ...
                                            <template x-if="move.ref_code">
                                                <template x-if="move.detail_url">
                                                    <a :href="move.detail_url" 
                                                       class="text-[10px] font-mono text-blue-600 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded w-fit hover:bg-blue-600 hover:text-white transition-all flex items-center gap-1 group/link">
    ...
    function movementApp() {
        return {
            ...
            async openModal(productId) {
                ...
                try {
                    const response = await fetch(`{{ route('admin.inventory.stock-movements.index') }}/${productId}/details?from=${from}&to=${to}`);
