                ...
                <div class="bg-white p-6 rounded-xl shadow-lg">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm">Pantau status desain yang berasal dari pesanan DTF dan
                                Jersey, lalu kirim update ke customer.</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow p-5">
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="text" name="search" value="{{ $search }}" placeholder="Contoh: SAL2412 / Rina"
                        ...
                        <div class="flex items-end gap-2">
                            <button
                                class="w-full bg-blue-600 text-white rounded-lg py-2 hover:bg-blue-700">Terapkan</button>
                            <a href="{{ route('editor.dashboard') }}"
                                class="w-full text-center bg-gray-100 text-gray-600 rounded-lg py-2 hover:bg-gray-200">Reset</a>
                        </div>
                    </form>
                </div>
                        ...
                        <div class="bg-white rounded-xl shadow border border-gray-100 flex flex-col">
                            ...
                            <div class="p-4 space-y-3 overflow-y-auto max-h-[360px]">
                                @forelse(($pipeline[$status] ?? collect()) as $item)
                                    @php
                                        // Logic Urgensi Visual
                                        $isStagnant = $item->design_status === 'pending' && $item->created_at->diffInDays(now()) > 2;
                                        $deadlineRisk = false;
                                        if ($item->salesOrder->deadline) {
                                            $daysLeft = now()->diffInDays($item->salesOrder->deadline, false);
                                            $deadlineRisk = $daysLeft <= 3 && $item->design_status !== 'approved';
                                        }
                                        $isRevisi = $item->design_status === 'rejected';
                                        
                                        // Card Class (Background)
                                        $cardClass = 'bg-white border-gray-100'; // Default
                                        if ($isStagnant) $cardClass = 'bg-red-50 border-red-200';
                                        if ($isRevisi) $cardClass = 'bg-orange-50 border-orange-200';
                                    @endphp

                                    <div class="border rounded-lg p-3 shadow-sm {{ $cardClass }} relative">
                                        {{-- Indikator Bahaya di Atas --}}
                                        @if($deadlineRisk)
                                            <div class="absolute -top-2 -right-2">
                                                <span class="animate-pulse bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                                                    RISK!
                                                </span>
                                            </div>
                                        @endif
                                        ...
                                        <div class="flex items-center justify-between mb-2">
                                            <p class="text-sm font-bold text-gray-800">{{ $item->salesOrder->so_number }}</p>
                                            <div class="flex flex-col items-end gap-1">
                                                @if($isRevisi)
                                                    <span class="text-[10px] font-bold bg-red-600 text-white px-2 py-0.5 rounded animate-pulse">REVISI</span>
                                                @else
                                                    <span class="text-[10px] px-2 py-0.5 rounded-full {{ $statusColors[$item->design_status] ?? 'bg-gray-100 text-gray-600' }}">
                                                        {{ substr(\App\Models\SalesOrderItem::designStatusOptions()[$item->design_status] ?? $item->design_status, 0, 10) }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>

                                        <p class="text-sm text-gray-700 font-medium leading-tight mb-1">{{ $item->product_name }}</p>
                                        <p class="text-xs text-gray-500 mb-2">
                                            {{ $item->salesOrder->customer->name ?? 'Customer Umum' }}
                                        </p>

                                        {{-- Pending Lama Warning --}}
                                        @if($isStagnant)
                                            <div class="mb-2 flex items-center gap-1 text-red-600 text-xs font-bold bg-white px-2 py-1 rounded border border-red-100">
                                                <i class="bi bi-exclamation-circle-fill"></i>
                                                Mangkrak {{ $item->created_at->diffInDays(now()) }} Hari!
                                            </div>
                                        @endif

                                        <div class="flex items-center justify-between mt-3 text-xs pt-2 border-t {{ $isStagnant ? 'border-red-200' : 'border-gray-100' }}">
                                            <div class="flex items-center gap-1 {{ $deadlineRisk ? 'text-red-600 font-bold' : 'text-gray-500' }}">
                                                <i class="bi bi-calendar-event"></i>
                                                {{ optional($item->salesOrder->deadline)->format('d M') ?? '-' }}
                                                @if($deadlineRisk) <span class="text-[10px] ml-1">(!H-{{ abs(now()->diffInDays($item->salesOrder->deadline, false)) }})</span> @endif
                                            </div>
                                            <a href="{{ route('editor.sales.show', $item->sales_order_id) }}"
                                ...
                                @empty
                                    <div class="text-center py-6">
                                        <div class="inline-flex p-3 rounded-full bg-gray-50 text-gray-300 mb-2">
                                            <i class="bi bi-inbox text-xl"></i>
                                        </div>
                                        <p class="text-sm text-gray-400">Kosong</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                ...
                <section class="bg-white rounded-xl shadow">
                    <div class="p-5 border-b flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Daftar lengkap order DTF dan Jersey yang membutuhkan
                                desain.</p>
                        </div>
                        <p class="text-sm text-gray-400">Menampilkan {{ $tasks->firstItem() }} -
                            {{ $tasks->lastItem() }} dari {{ $tasks->total() }} data</p>
                    </div>
                                    ...
                                    <tr class="align-top">
                                        <td class="px-5 py-4">
                                            ...
                                            <p class="text-xs text-gray-500">
                                                {{ $task->salesOrder->customer->name ?? 'Customer Umum' }}</p>
                                            <p class="text-xs text-gray-400 mt-1">
                                                Deadline:
                                                {{ optional($task->salesOrder->deadline)->format('d M Y') ?? '-' }}
                                            </p>
                                        </td>
                                        <td class="px-5 py-4">
                                            ...
                                            <td class="px-5 py-4">
                                                <div class="flex flex-col items-start gap-1">
                                                    ...
                                                    @if($task->design_status === 'pending')
                                                        @php $daysPendingTask = $task->created_at->diffInDays(now()); @endphp
                                                        @if($daysPendingTask > 2)
                                                            <span class="text-[10px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded border border-red-100">
                                                                <i class="bi bi-exclamation-circle-fill mr-1"></i>{{ $task->created_at->diffForHumans() }}
                                                            </span>
                                                        @else
                                                        ...
                                                        @endif
                                                    @else
                                                        <p class="text-xs text-gray-400">Update: {{ $task->updated_at->diffForHumans() }}</p>
                                                    @endif
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 space-y-1 text-xs">
                                            @if($task->design_reference_path)
                                                <a href="{{ Storage::url($task->design_reference_path) }}" target="_blank"
                                                    class="flex items-center gap-1 text-blue-600 hover:underline">
                                                    <i class="bi bi-cloud-arrow-down"></i> Brief
                                                </a>
                                            @endif
                                            @if($task->design_preview_path)
                                                <a href="{{ Storage::url($task->design_preview_path) }}" target="_blank"
                                        ...
                                        <td class="px-5 py-4">
                                            <details class="bg-gray-50 rounded-lg">
                                                <summary class="cursor-pointer px-3 py-2 text-sm font-medium text-blue-600">
                                                    Perbarui</summary>
                                                <div class="px-3 py-3 space-y-3 text-xs text-gray-600">
                                                    <form action="{{ route('editor.design-tasks.update', $task) }}"
                                                        ...
                                                        <div class="space-y-1">
                                                            ...
                                                            <select name="design_status"
                                                                ...
                                                                @foreach($designStatusOptions as $value => $label)
                                                                    <option value="{{ $value }}"
                                                                        @selected($task->design_status === $value)>{{ $label }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="space-y-1">
                                                            ...
                                                            <textarea name="design_brief" rows="2"
                                                                class="w-full border-gray-200 rounded-lg"
                                                                placeholder="Ringkasan kebutuhan">{{ old('design_brief', $task->design_brief) }}</textarea>
                                                        </div>
                                                        <div class="space-y-1">
                                                            <label class="text-xs font-semibold">Catatan Editor</label>
                                                            <textarea name="design_notes" rows="2"
                                                                class="w-full border-gray-200 rounded-lg"
                                                                placeholder="Progress terakhir">{{ old('design_notes', $task->design_notes) }}</textarea>
                                                        </div>
                                                        <div class="space-y-1">
                                                            <label class="text-xs font-semibold">Feedback Customer</label>
                                                            <textarea name="design_feedback" rows="2"
                                                                class="w-full border-gray-200 rounded-lg"
                                                                placeholder="Catatan customer">{{ old('design_feedback', $task->design_feedback) }}</textarea>
                                                        </div>
                                                        <div class="space-y-1">
                                                            <label class="text-xs font-semibold">Upload Brief</label>
                                                            <input type="file" name="design_reference"
                                                                class="w-full text-xs text-gray-500">
                                                        </div>
                                                        <div class="space-y-1">
                                                            <label class="text-xs font-semibold">Upload Preview</label>
                                                            <input type="file" name="design_preview"
                                                                class="w-full text-xs text-gray-500">
                                                        </div>
                                                        <button
                                                            class="w-full bg-blue-600 text-white rounded-lg py-1.5 text-sm hover:bg-blue-700">Simpan</button>
                                                    </form>
                                                </div>
                                            </details>
                                        </td>
                            ...
                            <div class="flex gap-3">
                                ...
                                <div>
                                    <p class="text-sm text-gray-800">
                                        {{ $activity->salesOrder->customer->name ?? 'Customer Umum' }} •
                                        {{ $activity->product_name }}
                                    </p>
                                    <p class="text-xs text-gray-500">
                                        Status:
                                        {{ \App\Models\SalesOrderItem::designStatusOptions()[$activity->design_status] ?? $activity->design_status }}
