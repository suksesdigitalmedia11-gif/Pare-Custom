                ...
                <div class="bg-white p-4 lg:p-6 rounded-xl shadow-lg mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('owner.purchases.import-form') }}"
                            ...
                            <a href="{{ route('owner.purchases.export', request()->query()) }}"
                        ...
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="border-b-2 border-gray-200 bg-gray-50">
                                    <th class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700">No. Pembelian
                                    </th>
                                    <th
                                        class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 hidden sm:table-cell">
                                        Tanggal</th>
                                    <th class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700">Tipe</th>
                                    <th
                                        class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 hidden md:table-cell">
                                        Supplier</th>
                                    <th
                                        class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 hidden lg:table-cell">
                                        Jumlah</th>
                                    <th
                                        class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 hidden xl:table-cell">
                                        Keterangan</th>
                                    <th class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700">Status</th>
                                    <th
                                        class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 hidden lg:table-cell">
                                        Progress</th>
                                    <th class="px-3 lg:px-4 py-3 text-left font-semibold text-gray-700 text-center">Aksi
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($purchases as $p)
                                    <tr class="border-b hover:bg-gray-50 transition-colors">
                                        <td class="px-3 lg:px-4 py-3">
                                            <a class="text-[#005281] hover:underline font-medium"
                                                href="{{ route('owner.purchases.show', $p) }}">{{ $p->po_number }}</a>
                                            <div class="text-xs text-gray-500 sm:hidden mt-1">
                                                {{ \Carbon\Carbon::parse($p->order_date)->format('d M Y') }}</div>
                                        </td>
                                        <td class="px-3 lg:px-4 py-3 hidden sm:table-cell">
                                            {{ \Carbon\Carbon::parse($p->order_date)->format('d M Y') }}</td>
                                        <td class="px-3 lg:px-4 py-3">
                                            <span
                                                class="px-2 py-1 rounded text-xs font-medium {{ $p->purchase_type === 'kain' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                                        ...
                                        <td class="px-3 lg:px-4 py-3 hidden md:table-cell">
                                            ...
                                            <div class="text-xs text-gray-500 lg:hidden">Rp
                                                {{ number_format($p->grand_total, 0, ',', '.') }}</div>
                                        </td>
                                        <td class="px-3 lg:px-4 py-3 hidden lg:table-cell font-medium">Rp
                                            {{ number_format($p->grand_total, 0, ',', '.') }}</td>
                                        ...
                                        <td class="px-3 lg:px-4 py-3 hidden lg:table-cell">
                                            @if($p->purchase_type === 'kain')
                                                ...
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        class="text-xs text-gray-600 font-medium">{{ round($progress) }}%</span>
                                                </div>
                                            @else
                                                @php
                                        ...
                                        <td class="px-3 lg:px-4 py-3">
                                            <div class="flex items-center justify-center gap-1 flex-wrap">
                                                <!-- Detail Button -->
                                                <a href="{{ route('owner.purchases.show', $p) }}"
                                                ...
                                                <!-- Draft: Submit -->
                                                @if($p->status === 'draft')
                                                    <form method="POST" action="{{ route('owner.purchases.submit', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                <!-- Approve -->
                                                @if($p->status === 'pending')
                                                    <form method="POST" action="{{ route('owner.purchases.approve', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                @if($p->status === 'payment')
                                                    @if($p->purchase_type === 'kain')
                                                        <form method="POST"
                                                            action="{{ route('owner.purchases.update-status', $p) }}"
                                                            class="inline">
                                                            @csrf
                                                ...
                                                @if(in_array($p->status, ['draft', 'pending', 'request_kain']))
                                                    <form method="POST" action="{{ route('owner.purchases.cancel', $p) }}"
                                                        class="inline" onsubmit="return confirm('Batalkan pembelian ini?')">
                                                        @csrf @method('PATCH')
                                                        <button
                                                ...
                                                @if(!in_array($p->status, ['selesai', 'payment', 'proses_jahit', 'printing']))
                                                    <form method="POST" action="{{ route('owner.purchases.destroy', $p) }}"
                                                        class="inline" onsubmit="return confirm('Hapus permanen purchase order ini? Semua data terkait akan dihapus.')">
                                                        @csrf @method('DELETE')
                                                        <button
                                                            class="px-2 py-1 text-xs bg-gray-600 text-white rounded hover:bg-gray-700 transition-colors"
                                                            title="Hapus">
                                                    ...
                                                    <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                                                        ...
                                                        <form action="{{ route('owner.purchases.payment', $p->id) }}"
                                                            ...
                                                            <div class="mb-4">
                                                                <label
                                                                    class="block text-sm font-medium text-gray-700 mb-1">Upload
                                                                    Bukti Pembayaran (PDF/JPG/PNG) - Opsional</label>
                                                            ...
                                                            <div class="flex justify-end space-x-2">
                                                                <button type="button"
                                                                    onclick="closeModal('payment-modal-{{ $p->id }}')"
