    ...
    <div class="flex-1">
      ...
      <div class="p-4 lg:p-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg" x-data="opnameForm()">
          <form action="{{ route($routePrefix . '.inventory.stock-opnames.store') }}" method="POST"
            @submit="submitForm">
            @csrf
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              <a href="{{ route($routePrefix . '.inventory.stock-opnames.index') }}"
      ...
      return {
        ...
        init() {
          ...
          if (!hasOldData) {
            ...
            if (savedDate) this.formData.date = savedDate;
          }

          if (this.items.length === 0) this.addItem();
          this.items.forEach(item => { item.searchResults = []; });

          this.$watch('items', (val) => localStorage.setItem('opname_draft_items', JSON.stringify(val)));
          this.$watch('formData.date', (val) => localStorage.setItem('opname_draft_date', val));
        },
        ...
        addItem() {
          this.items.push({
            product_id: '', product_name: '', sku: '', system_qty: 0, actual_qty: 0, searchResults: []
          });
        },
        ...
        async searchProduct(query, index, scope) {
          ...
          scope.loading = true;
          try {
            const response = await fetch(`{{ route($routePrefix . '.inventory.stock-opnames.search-products') }}?q=${query}`);
        ...
        selectProduct(index, product) {
          ...
          item.actual_qty = product.stock_qty;
          item.searchResults = [];
        },
        ...
        submitForm(e) {
          ...
          if (this.items.some(item => !item.product_id)) {
            alert('Mohon pilih produk yang valid.');
            e.preventDefault();
