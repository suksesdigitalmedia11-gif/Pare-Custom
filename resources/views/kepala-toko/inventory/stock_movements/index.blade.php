      ...
      <div class="p-4 lg:p-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                <div class="w-10 h-10 bg-[#005281] rounded-lg flex items-center justify-center text-white shadow-lg shadow-blue-200">
            ...
            <div class="flex border-b border-gray-200 mt-6 overflow-x-auto bg-white rounded-t-xl px-4 pt-2">
                <a href="{{ route('kepala-toko.inventory.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('kepala-toko.inventory.index') ? 'border-b-2 border-[#005281] text-[#005281]' : 'text-gray-500 hover:text-[#005281]' }}">Overview</a>
                <a href="{{ route('kepala-toko.inventory.stock-ins.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('kepala-toko.inventory.stock-ins.*') ? 'border-b-2 border-[#005281] text-[#005281]' : 'text-gray-500 hover:text-[#005281]' }}">Stok Masuk</a>
                <a href="{{ route('kepala-toko.inventory.stock-opnames.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('kepala-toko.inventory.stock-opnames.*') ? 'border-b-2 border-[#005281] text-[#005281]' : 'text-gray-500 hover:text-[#005281]' }}">Stock Opname</a>
                <a href="{{ route('kepala-toko.inventory.stock-movements.index') }}" 
                   class="px-6 py-3 font-semibold whitespace-nowrap transition-all {{ request()->routeIs('kepala-toko.inventory.stock-movements.*') ? 'border-b-2 border-[#005281] text-[#005281]' : 'text-gray-500 hover:text-[#005281]' }}">Pergerakan Stok</a>
            </div>
        </div>
            ...
            <form method="GET" class="flex flex-col lg:flex-row gap-4">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ $search }}" 
                        class="w-full pl-11 pr-4 py-3 rounded-xl border-gray-200 focus:ring-2 focus:ring-[#005281] focus:border-[#005281] transition-all shadow-sm" 
                        placeholder="Cari Produk atau SKU...">
                </div>
                <div class="flex flex-wrap lg:flex-nowrap gap-3">
                    <div class="relative group">
                        <label class="absolute -top-2 left-3 px-1 bg-white text-[10px] font-bold text-[#005281] uppercase tracking-wider">Mulai</label>
                        <input type="date" name="from" value="{{ $from }}" 
                            class="rounded-xl border-gray-200 py-3 text-sm focus:ring-[#005281] focus:border-[#005281] shadow-sm transition-all">
                    </div>
                    <div class="relative group">
                        <label class="absolute -top-2 left-3 px-1 bg-white text-[10px] font-bold text-[#005281] uppercase tracking-wider">Sampai</label>
                        <input type="date" name="to" value="{{ $to }}" 
                            class="rounded-xl border-gray-200 py-3 text-sm focus:ring-[#005281] focus:border-[#005281] shadow-sm transition-all">
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="bg-[#005281] text-white px-6 py-3 rounded-xl hover:bg-[#004269] transition-all shadow-lg shadow-blue-100 font-bold flex items-center gap-2">
          ...
          <div class="table-container">
              <table class="min-w-full text-sm text-left border-separate border-spacing-0">
                  ...
                  @forelse($stockMovements as $item)
                      <tr class="hover:bg-blue-50/30 transition-all group cursor-pointer" @click="openModal({{ $item->product_id }})">
                          <td class="px-6 py-4">
                              <div class="font-bold text-gray-900 leading-tight">{{ $item->product->name ?? 'Produk Dihapus' }}</div>
                              <div class="text-[11px] font-medium text-[#005281] mono mt-1 select-all group-hover:underline">
                                  {{ $item->product->sku ?? '-' }}
                              </div>
                          </td>
                          ...
                          <td class="px-6 py-4 text-right">
                              <button class="w-8 h-8 rounded-full bg-white border border-gray-200 text-gray-400 flex items-center justify-center hover:bg-[#005281] hover:text-white hover:border-[#005281] transition-all shadow-sm">
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full"
            ...
            <div class="px-6 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-blue-100 rounded-2xl flex items-center justify-center text-[#005281] shadow-inner">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5 mono">
                            SKU: <span class="font-bold text-[#005281]" x-text="details.product.sku"></span> | 
            ...
            <div class="max-h-[60vh] overflow-y-auto px-6 py-4">
                <div x-show="isLoading" class="flex flex-col items-center justify-center py-20">
                    <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-[#005281]"></div>
                                            ...
                                            <template x-if="move.ref_code">
                                                <template x-if="move.detail_url">
                                                    <a :href="move.detail_url" 
                                                       style="color: #005281; background-color: rgba(0, 82, 129, 0.05); border-color: rgba(0, 82, 129, 0.1);"
                                                       class="text-[10px] font-mono border px-1.5 py-0.5 rounded w-fit hover:bg-[#005281] hover:text-white transition-all flex items-center gap-1 group/link">
    ...
    function movementApp() {
        return {
            ...
            async openModal(productId) {
                ...
                try {
                    const response = await fetch(`{{ route('kepala-toko.inventory.stock-movements.index') }}/${productId}/details?from=${from}&to=${to}`);
