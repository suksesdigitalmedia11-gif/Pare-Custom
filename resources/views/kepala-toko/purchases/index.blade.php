                ...
                <div class="bg-white p-4 lg:p-6 rounded-xl shadow-lg mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('kepala-toko.purchases.import-form') }}"
                            ...
                            <a href="{{ route('kepala-toko.purchases.export', request()->query()) }}"
                                    ...
                                    <tr class="border-b hover:bg-gray-50 transition-colors">
                                        <td class="px-3 lg:px-4 py-3">
                                            <a class="text-[#005281] hover:underline font-medium"
                                                href="{{ route('kepala-toko.purchases.show', $p) }}">{{ $p->po_number }}</a>
                                        ...
                                        <td class="px-3 lg:px-4 py-3">
                                            <div class="flex items-center justify-center gap-1 flex-wrap">
                                                ...
                                                <!-- Draft: Submit -->
                                                @if($p->status === 'draft')
                                                    <form method="POST" action="{{ route('kepala-toko.purchases.submit', $p) }}"
                                                        class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                @if(count($availableStatuses) > 0 && !in_array($p->status, ['draft', 'pending', 'request_kain']))
                                                    @foreach($availableStatuses as $nextStatus)
                                                        @if(in_array($nextStatus, ['proses_jahit', 'printing', 'selesai']) && in_array(auth()->user()->usertype, ['kepala_toko', 'owner']))
                                                            <form method="POST"
                                                                action="{{ route('kepala-toko.purchases.update-status', $p) }}"
                                                                class="inline">
                                                                @csrf
                                                                <input type="hidden" name="new_status" value="{{ $nextStatus }}">
                                                                <button
                                                                    class="px-2 py-1 text-xs bg-[#005281] text-white rounded hover:opacity-90 transition-colors"
                                                ...
                                                @if(!in_array($p->status, ['selesai', 'cancelled', 'payment', 'proses_jahit', 'printing']))
                                                    <form method="POST" action="{{ route('kepala-toko.purchases.cancel', $p) }}"
