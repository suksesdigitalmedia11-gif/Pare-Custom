@php
    $menuItems = [
        ...
        [
            ...
            'icon' => 'bi-sliders',
            'route' => route('editor.inventory.stock-adjustments.index'),
            'active' => request()->routeIs('editor.inventory.stock-adjustments.*'),
        ],
    ];
@endphp
...
<body>
    ...
    <div id="sidebar"
        ...
        <div class="flex-1 overflow-y-auto sidebar-scroll py-4">
            <nav class="space-y-1 px-4">
                @foreach($menuItems as $item)
                    <a href="{{ $item['route'] }}"
                        class="nav-item flex items-center px-4 py-3 rounded-lg {{ $item['active'] ? 'active' : 'text-gray-700 hover:bg-gray-100' }}">
        ...
        <form action="{{ route('logout') }}" method="POST" class="p-4 border-t border-gray-200 bg-white">
            @csrf
            <button type="submit"
                class="w-full flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg py-2">
